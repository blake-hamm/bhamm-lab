# Hardware

## Overview
This page provides a view of the lab's physical infrastructure. It covers the hardware inventory and future expansion plans.

## Hardware Inventory

### Servers

| Name     | Model                          | CPU                          | RAM       | Storage                                      | Features                               | Role                           |
|----------|--------------------------------|------------------------------|-----------|----------------------------------------------|----------------------------------------|--------------------------------|
| Method   | Supermicro H12SSL-i            | AMD EPYC 7502    | 128 GB ECC     | 128GB SSD (boot), 1.5TB Intel NVMe (ceph db/wal), 1TB NVMe (VM storage via local-vg), 1TB SSD (ceph osd), 3.84TB SSD (ceph osd), 1TB SSD (ceph osd) | Dual 10 GB NIC SFP+, ipmi, AMD Radeon AI Pro R9700, Intel Arc A310e, Intel Arc Pro B70 (Battlemage), SATA HBA | Proxmox, ceph, NUT client |
| Indy     | Supermicro D-2146NT            | Intel Xeon D-2146NT 8-Core   | 128 GB ECC| 1TB SATA SSD (boot, `indy-vg`), 1TB NVMe (VM storage, `local-vg`), 1.4TB Intel Optane NVMe (ceph db/wal), 1TB SSD (ceph osd), 3.84TB SSD (ceph osd), 7TB SSD (ceph osd) | 2x10GB NIC SFP+, 2x10GB NIC, 2x1GB NIC, ipmi | Proxmox, ceph, NUT client |
| Japan    | X10SDV-4C-TLN4F                | Intel Xeon CPU D-1518 8-Core | 64 GB ECC     | 240GB SATA SSD (boot, `japan-vg`), 1TB NVMe (VM storage, `local-vg`), 1.5TB Intel NVMe (ceph db/wal), 1TB SSD (ceph osd), 3.84TB SSD (ceph osd), 3.5TB SSD (ceph osd) | 2x10GB NIC, 2x1GB NIC, ipmi           | Proxmox, ceph, NUT client      |
| Stale    | X10SDV-4C-TLN4F                | Intel Xeon CPU D-1518 8-Core | 64 GB ECC     | 1TB NVME (boot and vm storage), 1 TB NVME (ceph db/wal), 1TB SSD (ceph osd), 3.84 TB SSD (ceph osd) | 2x10GB NIC, 2x1GB NIC, ipmi           | Proxmox and ceph               |
| Nose     | Framework Mainboard            | AMD Ryzen AI MAX+ 395        | 128 GB (120 dedicated vram)     | 500GB NVME (boot drive), 1 TB NVME (model local path storage)           | 5GB NIC, Thunderbolt | Talos bare metal, NUT client |
| Tail     | Framework Mainboard            | AMD Ryzen AI MAX+ 395        | 128 GB (120 dedicated vram)     | 500GB NVME (boot drive), 1 TB NVME (model local path storage)           | 5GB NIC, Thunderbolt | Talos bare metal, NUT client |

### Networking Equipment

| Name         | Model           | Specifications                                      | Role                        |
|--------------|-----------------|-----------------------------------------------------|-----------------------------|
| PoE Switch   | SG3218XP-M2     | Omada 16-Port 2.5GBASE-T and 2-Port 10GE SFP+ L2+ Managed Switch with 8-Port PoE+ | PoE and core network switch |
| Server Switch  | TL-SX3008F      | JetStream 8-Port 10G SFP+ L2+ Managed Switch        | Ceph and core server switch |
| Access Point       | EAP650          | WiFi 6 support                                      | WiFi network                |

### Single Board Computers

| Name                   | Model           | Architecture | IP                            | Network | Role                           |
|------------------------|-----------------|--------------|-------------------------------|---------|--------------------------------|
| Orange Pi Zero3        | Orange Pi Zero3 | aarch64/ARM  | 10.0.9.3 (static) + 10.0.9.2 (VIP) | LAN     | Pi-hole DNS (Primary) + NUT UPS Server |
| Orange Pi Zero3 Backup | Orange Pi Zero3 | aarch64/ARM  | 10.0.9.4 (static) + 10.0.9.2 (VIP) | LAN     | Pi-hole DNS (Backup) + NUT UPS Server |

### Peripheral Devices
- **UPS (Primary):** CyberPower OR1500LCDRT2U Smart App LCD UPS
- **UPS (Backup):** CyberPower OR1500LCDRT2U Smart App LCD UPS
- **PiKVM:** KVM for managing physical devices

## Future Plans
- **Replace Antsle:** The antsle server is the weakest link in my setup. At some point, I'd like to replace it with something that has more PCIe expandability. Then, I would move my Antsle server to my parents for an offsite backup and site-to-site VPN.
