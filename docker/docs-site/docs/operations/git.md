...
The prod cluster runs a self-hosted [Forgejo Actions](https://forgejo.org/docs/latest/admin/actions/) runner so this repository can use native Actions workflows (see `.forgejo/workflows/`).

- **Namespace:** `git-actions` (prod only), `pod-security.kubernetes.io/enforce: privileged` — the runner pod runs a docker-in-docker sidecar and job containers on the host network.
- **Chart:** `code.forgejo.org/forgejo-helm/forgejo-runner.git`, pinned to a commit SHA in the `git-actions` ArgoCD app (`kubernetes/manifests/core/forgejo/runner-helm-prod.yaml`). The chart has no tags, releases, or OCI publication.
...
- **Smoke test:** `.forgejo/workflows/smoke-test.yml` runs on push to any branch (the Forgejo repo is a Codeberg mirror — a Codeberg webhook triggers an immediate mirror sync, which fires a push event; PRs are never mirrored) and on manual dispatch; it has no external action dependencies and validates the runner, dind, and job image pull end-to-end.

### Rotating the registration token

1. In the Forgejo UI: **Settings → Actions → Runners** — delete the old runner, create a new one (name `prod`), copy the one-time registration token.
2. Encrypt it into `secrets.enc.json` under `vault_secrets.default.forgejo-runner.REGISTRATION_TOKEN` (SOPS/GCP KMS) and commit.
