...
2. **Multiplex authz relied on a backported monkey-patch** (`authz-patch-configmap-prod.yaml`, upstream PR #44706, unmerged). Under multiplex the group allowlist read fell through to process-global `os.getenv` when a profile had no `.env` — leaking profile A's allowlist across profiles (issue #72348 pattern).
...
Files written: `kubernetes/manifests/apps/ai/hermes/{namespace,common,deploy,service,networkpolicy}-prod.yaml`. Discovery automatic via `prod-apps` Application (`apps/**`, `{**all.yaml,**prod.yaml}`) — no manual registration needed. Preview flow: `prod-core` + `prod-apps` + hermes' own Application pointed at `feature/hermes` branch (whole core+apps stacks track the branch while open; revert to `main` by editing `base/core-prod.yaml` + `core/apps-prod.yaml` once merged).
...
1. ~~Register `hermes` app~~ — not needed, `prod-apps` Application auto-discovers `apps/**/*{all,prod}.yaml`.
